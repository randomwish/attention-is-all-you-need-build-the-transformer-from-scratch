...
def build_token_to_id_vocab(sentences, specials=('<pad>', '<bos>', '<eos>', '<unk>')):
    ...
    for idx, tok in enumerate(specials):
        new_dictionary[tok] = idx
    special_count = len(specials)
    split_sentences = itertools.chain(*[sentence.split() for sentence in sentences])
    for _, tok in enumerate(split_sentences):
        if tok not in new_dictionary:
            new_dictionary[tok] = special_count
            special_count += 1
...
def encode_sentence_to_ids(sentence, token_to_id, unk_token='<unk>'):
    ...
    for word in sentences_words:
        if word in token_to_id.keys():
            new_list.append(token_to_id[word])
        else:
            new_list.append(token_to_id[unk_token])
    return new_list
...
def pad_id_sequence(ids, max_len, pad_id):
    ...
    if len(new_id) < max_len:
        new_id += [pad_id] * (max_len - len(new_id))
    return new_id
...
def compute_positional_div_term(d_model):
    ...
    entries = []
    for entry in range(0, d_model // 2):
        entries.append(math.exp(2 * entry * (-math.log(10000) / d_model)))
...
def build_position_index_column(max_len):
    ...
    tensor_list = [torch.tensor([float(idx)]) for idx in range(max_len)]
...
def build_sinusoidal_positional_encoding(max_len, d_model):
    ...
    base_matrix = torch.zeros((max_len, d_model))
    div_term = compute_positional_div_term(d_model)
    pos_col = build_position_index_column(max_len)
    even_mat = fill_even_indices_with_sin(base_matrix, pos_col, div_term)
...
def build_padding_mask(token_ids, pad_id):
    ...
    shape_token = token_ids.shape
    bool_mask = torch.ne(token_ids, pad_id)
...
def build_causal_mask(seq_len):
    ...
    bool_mask = torch.zeros((seq_len,seq_len), dtype=torch.bool)
    for idx in range(seq_len):
        for iterate in range(idx, seq_len):
            bool_mask[iterate][idx] = True
...
def softmax_attention_weights(masked_scores):
    ...
    soft_attn = torch.softmax(masked_scores, dim = -1)
    return torch.nan_to_num(soft_attn, nan=0.0)
...
def scaled_dot_product_attention(query, key, value, mask=None):
    ...
    scaled = scale_attention_scores(raw_scores, d_k)
    if mask is not None:
        scaled = mask_attention_scores_with_neg_inf(scaled, mask)
    softmax = softmax_attention_weights(scaled)
    context = apply_attention_weights_to_values(softmax, value)
...
def split_last_dim_into_heads(tensor, num_heads):
    ...
    return tensor.reshape(tensor.shape[0], tensor.shape[1], num_heads, tensor.shape[2] // num_heads)
...
def merge_heads_back_to_model_dim(multi_head_tensor):
    ...
    tensor = multi_head_tensor.permute(0,2,1,3)
    shape = tensor.shape
    return tensor.reshape(shape[0], shape[1], shape[2] * shape[3])

# Step 26 - apply_linear_projection
def apply_linear_projection(x, weight, bias):
    # TODO: return x @ weight^T + bias (bias may be None) with shape (..., out_features)
    w_transpose = weight.T
    if bias == None:
        bias = 0
    return x @ w_transpose + bias
...
def split_qkv_into_heads(q, k, v, num_heads):
    ...
    list_split = [q_split, k_split, v_split]
    list_split = [transpose_heads_before_sequence(split) for split in list_split]
...
def merge_heads_and_project_output(context, w_o, b_o):
    ...
    return apply_linear_projection(merged_heads, w_o, b_o)
